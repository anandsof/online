...
// Here for the Publisher or Normal User or Power user
...
// TO make power User
  ...
  if ($egroup)
  {
	for($count=0; $count < count($egroup)-1 ; $count++)
	{
	$gp = $egroup[$count];
...
<br>
<form action="addUserGroup.php" name="form" method="post" onsubmit="return validate_form()">
...
print("\n<TR><TD colspan=2><BR>Select the exam which this user will have permission to add, edit and delete questions from:</TD></TR>\n");
   ...
   while($grow = mysqli_fetch_row($res))
	print("\n<tr>\n<td class=label align=\"right\"></td>\n<td><input name=\"egroup[]\" type=\"checkbox\" value=\"$grow[0]\" $check>$grow[1]</td>\n</tr>\n");
