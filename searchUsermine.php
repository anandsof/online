...
<form name="Search"  onSubmit="return validate_Search_form()"          method=POST ACTION="usermine.php">
...
?>
  ...
  <tr align="center"> 
    <td> 
        <table width=100%>
          <tr> 
            <td class=label align=right>User Level</td>
            <td nowrap> 
              <select name="user">
...
<?php  { ?>
          <tr> 
            <td class=label align=right>Groups</td>
            <td nowrap> 
              <select name="group">
                <option selected value="All"> All
<?php 
if($admin)
  $result =  mysqli_query($db_connect,"SELECT * FROM groups ORDER BY groupid");
else
  $result = mysqli_query($db_connect,"select groups.groupid, groupName from groupPermissions, groups where groups.groupid = groupPermissions.groupid and candidateID='$ID' ");
...
<?php } ?>
	...
	<tr> 
            ...
            <td> 
              <input type=text name="candidateID" size=10>
