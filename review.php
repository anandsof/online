	...
	function goToQuestion(form)
		{
			...
			if ((questionNo !=null) && (questionNo !=""))
			{
				if (isNumberString(questionNo))
				{
					...
					{
						location.href="review.php?scoreid="+document.form.scoreid.value+"&Page="+questionNo ;
					}
					else
					{
...
<form name="form" action="review.php" method="post">
...
$submit = $_POST['submit']; 
 ...
 $q1 = mysqli_query($db_connect,"SELECT questionid, answer, marks FROM temp where scoreid ='$scoreid' ORDER BY questionid");
...
ENDREVIEW;
 if($val[14] == 'dd')
 {
  print ("<tr><td valign=top width=10%><font face=\"Arial\">Correct Answer:</font></td>");
$di = 0;
print ("<td width=90%><table  cellpading=0 cellspacing=5>");
...
// if Image
 if ($imageid)
 {
  print ("<tr>
      ...
      <td width=\"90%\"><font color=RED><a href=\"javascript: onclick=MM_openBrWindow('picture.php?imageid=$imageid','Image','width=$width, height=$height' )\">$description</a></font><br><br></td>
</tr>");
  }
...
<font face="Arial">Explanation:</font></td>
</tr>
<tr>
<td valign =top width=10%></td>
