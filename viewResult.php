...
function reviewTest(form)
{
  ...
  examWin.location = "viewReview.php?scoreid="+scoreid;
}
...
$query = mysqli_query($db_connect,"select question.answer as correct , test.answer as answer, test.markQuestion, test.questionid, type, category, userName, scores.candidateID from test join question join scores join candidate where test.questionid=question.questionid and test.scoreid = scores.scoreid and scores.candidateID=candidate.candidateID  and test.scoreid = '$scoreid' ORDER BY questionid");
...
$q6 = mysqli_query($db_connect,"SELECT questionid, marks, answer FROM test WHERE scoreid='$scoreid' order by questionid ");
$k = 0;
...
<form method="post" action="viewReview.php" name="form"> 
...
{
	location="viewResult.php?examid=<?php print("$examid&scoreid=$scoreid&time=$time&action=summ") ?>";
}

function resultCat(form)
{
	location="viewResult.php?examid=<?php print("$examid&scoreid=$scoreid&time=$time") ?>";
}
